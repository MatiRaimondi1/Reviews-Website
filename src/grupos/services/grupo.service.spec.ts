...
describe('GrupoService', () => {
    ...
    describe('delete', () => {
        ...
        it('Debería lanzar ForbiddenException si el usuario no es líder ni admin', async () => {
            ...
            await expect(service.delete(grupoId, userId)).rejects.toThrow(ForbiddenException);
        ...
        it('Debería lanzar NotFoundException si no existe el grupo', async () => {
            ...
            await expect(service.delete(grupoId, userId)).rejects.toThrow(NotFoundException);
    ...
    it('debe devolver mensaje que el usuario pertenece al grupo cuando la relación existe', async () => {
        repoMembresias.findOne.mockResolvedValue({} as MembresiaGrupo);
        ...
        expect(result).toEqual({ mensaje: 'El usuario pertenece al grupo.' });
    });
    ...
    it('debe devolver mensaje que el usuario no pertenece al grupo cuando la relación no existe', async () => {
        ...
        expect(result).toEqual({ mensaje: 'El usuario no pertenece al grupo.' });
    });

    it('debe lanzar NotFoundException si el grupo no existe', async () => {
        repoGrupos.findOneBy.mockResolvedValue(null);

        await expect(service.update(1, 1, { nombre: 'Nuevo nombre' }))
            .rejects
    ...
    describe('edit', () => {
        ...
        it('debe lanzar NotFoundException si el usuario no existe', async () => {
            ...
            await expect(service.update(1, 2, { nombre: 'Nuevo nombre' }))
                .rejects
                .toThrow(NotFoundException);
        });
        ...
        it('debe actualizar el grupo si el usuario es líder', async () => {
            ...
            repoUsers.findOneBy.mockResolvedValue(userMock);
            repoGrupos.save.mockResolvedValue({ ...grupoOriginal, nombre: 'Nuevo nombre' });

            const cambios = { nombre: 'Nuevo nombre' };
            const resultado = await service.update(1, 2, cambios);

            expect(repoGrupos.save).toHaveBeenCalledWith(expect.objectContaining({ nombre: 'Nuevo nombre' }));
        ...
        it('debe actualizar el grupo si el usuario es admin', async () => {
            ...
            repoGrupos.save.mockResolvedValue({ ...grupoOriginal, descripcion: 'Nueva descripción' });

            const cambios = { descripcion: 'Nueva descripción' };
            const resultado = await service.update(1, 2, cambios);

            expect(repoGrupos.save).toHaveBeenCalledWith(expect.objectContaining({ descripcion: 'Nueva descripción' }));
            expect(resultado).toEqual({
                mensaje: 'Grupo actualizado correctamente',
    ...
    it('debería expulsar al usuario correctamente si el solicitante es líder o admin', async () => {
        repoGrupos.findOneBy.mockResolvedValue({ id: 1, nombre: 'Grupo1' } as Grupo);
        ...
        expect(result).toEqual({ mensaje: 'Usuario expulsado correctamente del grupo.' });
    });
    ...
    it('debería lanzar NotFoundException si el usuario solicitante no existe', async () => {
        repoGrupos.findOneBy.mockResolvedValue({ id: 1 } as Grupo);
        repoMembresias.findOne.mockResolvedValue({ rol: 'lider' } as MembresiaGrupo);
        repoUsers.findOneBy.mockResolvedValue(null);
    ...
    it('debería lanzar ForbiddenException si el solicitante no es líder ni admin', async () => {
        repoGrupos.findOneBy.mockResolvedValue({ id: 1 } as Grupo);
        repoMembresias.findOne.mockResolvedValue({ rol: 'miembro' } as MembresiaGrupo);
