...
export class GrupoService {
    ...
    async update(grupoId: number, userId: number, cambios: { nombre?: string; descripcion?: string }) {
        ...
        if (cambios.descripcion !== undefined) grupo.descripcion = cambios.descripcion;
    ...
    async delete(grupoId: number, userId: number) {
        ...
        if (!esLider && !esAdmin) {
            throw new ForbiddenException('Solo el líder o un administrador puede eliminar el grupo');
        }
    ...
    async kickUser(grupoId: number, userIdExpulsar: number, userIdSolicitante: number) {
        ...
        const solicitante = await this.userRepo.findOneBy({ id: userIdSolicitante });
        if (!solicitante) {
        ...
        if (!relacionExpulsar) {
            throw new NotFoundException('El usuario a expulsar no pertenece al grupo');
        }
    ...
    async leave(grupoId: number, userId: number) {
        ...
        if (!relacion) {
            throw new NotFoundException('No estás en este grupo o el grupo no existe.');
        }
        ...
        if (miembrosRestantes === 0) {
            ...
            return { mensaje: 'Saliste del grupo. El grupo fue eliminado porque no tenía más miembros.' };
        }
    ...
    async isUserInGroup(userId: number, grupoId: number) {
        ...
        if (relacion) {
            return { mensaje: 'El usuario pertenece al grupo.' };
        } else {
            return { mensaje: 'El usuario no pertenece al grupo.' };
        }
    }
