...
export class ReviewsService {
    ...
    if (grupoId) {
      ...
      if (!miembro || miembro.rol !== 'lider') {
        throw new ForbiddenException(
          'Solo el lider del grupo puede crear una review grupal.',
        );
      }
      ...
      if (existingGroupReview) {
        throw new ConflictException(
          'Ya existe una review grupal para esta pelicula.',
        );
      }
      ...
      if (existingReview) {
        throw new ConflictException(
          'Ya existe una review para esta pelicula por este usuario.',
        );
      }
  ...
  async edit(
    id: number,
    userId: number,
    updateData: { texto?: string; puntuacion?: number },
  ) {
    ...
    if (!esAutor && !esAdmin) {
      throw new BadRequestException(
        'No puedes editar una reseña que no has creado',
      );
    }
  ...
  async delete(id: number, userId: number) {
    ...
    if (!esAutor && !esAdmin) {
      throw new BadRequestException(
        'No puedes eliminar una reseña que no has creado',
      );
    }
